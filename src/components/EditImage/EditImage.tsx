...
import {
  ...
  dragElement,
  saveState,
} from "../../functions/image-processing";
...
const EditImage = memo(
  ({
    ...
    const [croppSize, setCroppSize] = useState<{
      width: number;
      height: number;
    ...
    async function applyChanges(stateIntance: IState, changeHeight = false) {
      try {
        ...
        console.log(
          "🚀 ~ file: EditImage.tsx ~ line 73 ~ applyChanges ~ error",
          error
        );
      }
    }
    ...
    function onCrop() {
      ...
      return new Promise((resolve, reject) => {
        ...
        image.onload = () => {
          ...
          return resolve(
            canvas.toDataURL(`image/${newState.format}`, newState.quality)
          );
        };
        image.onerror = (e) => {
          reject(e);
    ...
    async function onRestore() {
      try {
        ...
        if (newState.arrayCopiedImages.length > 1) {
          ...
          let newValue =
            newState.arrayCopiedImages[newState.arrayCopiedImages.length - 1];
          newState = {
            ...state,
    ...
    async function onChangeFilters(data: IBasicFilterState) {
      try {
        ...
        console.log(
          "🚀 ~ file: EditImage.tsx ~ line 259 ~ onChangeFilters ~ e",
          e
        );
      }
    }
